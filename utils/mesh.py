...
def load_off(filename):
    """
    Parses an Object File Format (.off) file.
    ...
    Returns:
        tuple: (VPos [N,3], VColors [N,3], ITris [M,3 or M,N])
    """
    """
...
def load_obj(filename):
    ...
    Supports vertices (v), vertex colors (v x y z r g b), and faces (f) or lines (l).
    
    Args:
        filename (str): Path to .obj file.
        
    Returns:
        tuple: (VPos [N,3], VColors [N,3], ITris/Lines [M, K])
    """
    """
    Load in an OBJ file, reading vertices and lines (for graphs)
    Parameters
    ----------
    filename: string
        Path to file
    ...
    VColors : ndarray(N, 3)
        Array of RGB colors
    ILines : ndarray (M, 2)
        Array of lines connecting points
    """
    VPos = []
    VColors = [] 
    ILines = []
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            parts = line.split()
            if parts[0] == 'v':
                VPos.append([float(x) for x in parts[1:4]])
                if len(parts) >= 7:
                    VColors.append([float(x) for x in parts[4:7]])
            elif parts[0] == 'l':
                indices = [int(x) - 1 for x in parts[1:]]
                for i in range(len(indices) - 1):
                    ILines.append([indices[i], indices[i+1]])
    
    VPos = np.array(VPos, dtype=np.float64)
    if len(VColors) > 0:
        VColors = np.array(VColors, dtype=np.float64)
    else:
        VColors = np.zeros_like(VPos) + 0.5
        
    ILines = np.array(ILines, dtype=np.int32)
    return (VPos, VColors, ILines)

def save_off(filename, VPos, VColors, ITris):
    """
    Saves mesh data to an .off file.
    Supports writing colors (appended to vertices or faces).
    
    Args:
        filename (str): Output path.
        VPos (np.ndarray): [N,3] Vertices.
        VColors (np.ndarray): [N,3] Vertex Colors.
        ITris (np.ndarray): [M, K] Connectivity (Triangles=3, Lines=2).
    """
    """
    Save a .off file
    Parameters
    ...
    d = ITris.shape[1]
    for i in range(nF):
        fout.write(("%i" + " %i"*d + "\n")%tuple([d] + list(ITris[i, :])))
    fout.close()

def save_obj(filename, VPos, VColors, ITris):
    """
    Saves mesh data to a Wavefront .obj file.
    Writes vertices with colors (non-standard but common extension) and connectivity features.
    Automatically detects if ITris contains Lines (2 indices) or Faces (3+ indices).
    
    Args:
        filename (str): Output path.
        VPos (np.ndarray): [N,3] Vertices.
        VColors (np.ndarray): [N,3] Vertex Colors (0-1 range).
        ITris (np.ndarray): [M, K] Connectivity.
    """
    """
    Save a .obj file
    """
    ...
    with open(filename, "w") as fout:
        for i in range(nV):
            fout.write("v %g %g %g"%tuple(VPos[i, :]))
            if VColors.size > 0:
                fout.write(" %g %g %g"%tuple(VColors[i, :]))
            fout.write("\n")
        ...
        if nF > 0:
            d = ITris.shape[1]
            prefix = 'l' if d == 2 else 'f'
            for i in range(nF):
                indices = ITris[i] + 1
                fout.write(prefix + " " + " ".join(map(str, indices)) + "\n")
    

def sample_by_area(VPos, ITris, npoints, colPoints = False):
    """
    Randomly samples points from a triangle mesh surface proportional to triangle areas.
    Uses barycentric coordinates for uniform sampling inside triangles.
    
    Args:
        VPos (np.ndarray): [N, 3] Vertices.
        ITris (np.ndarray): [M, 3] Triangles.
        npoints (int): Number of points to sample.
        colPoints (bool): Return format flag.
        
    Returns:
        tuple: (SampledPoints [npoints, 3], SampledNormals [npoints, 3])
    """
    """
...
def get_edges(VPos, ITris):
    """
    Extracts unique edges from a triangle mesh.
    ...
    Returns:
        tuple (I, J): Parallel arrays of indices (i, j) for edges.
    """
    """
