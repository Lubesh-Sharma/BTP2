...
import argparse
from .mesh import load_off, save_off, load_obj, save_obj

def get_cotan_laplacian(VPos, ITris, anchorsIdx = [], anchorWeights = 1):
    """
    Computes the weighted Cotangent Laplacian matrix for a triangle mesh.
    
    The Cotangent Laplacian is L_ij = 0.5 * (cot(alpha_ij) + cot(beta_ij)).
    It provides a discretization of the Laplace-Beltrami operator on meshes.
    
    Args:
        VPos (np.ndarray): [N, 3] Vertex positions.
        ITris (np.ndarray): [M, 3] Triangle indices.
        anchorsIdx (list): Indices of anchor points constraints.
        anchorWeights (float): Weight for the anchor constraints.
        
    Returns:
        scipy.sparse.csc_matrix: [N+Anchors, N] Sparse Laplacian matrix.
    """
    """
...
def get_graph_laplacian(VPos, Edges):
    """
    Computes the combinatorial Graph Laplacian for point clouds connected by edges (lines).
    ...
    Args:
        VPos (np.ndarray): [N, 3] Vertex positions.
        Edges (np.ndarray): [M, 2] Edge indices.
        
    Returns:
        scipy.sparse.csc_matrix: [N, N] Graph Laplacian matrix.
    """
    """
    Compute sparse Graph Laplacian matrix (L = D - A)
    """
    N = VPos.shape[0]
    M = Edges.shape[0]
    
    # Edges are [i, j]. We assume undirected.
    ...
    # Normalize weights to 1 just in case of duplicates
    A.data[:] = 1
...
def get_hks(VPos, ITris, K, ts):
    ...
    print("Computing Laplacian (Graph or Cotan based on connectivity)...")
    if ITris.shape[1] == 2:
        L = get_graph_laplacian(VPos, ITris)
    else:
        L = get_cotan_laplacian(VPos, ITris)
    
    # Use a small shift to avoid singularity of the Laplacian at 0
    (eigvalues, eigvectors) = eigsh(L, K, which='LM', sigma = 1e-8)
...
def saveHKSColors(filename, VPos, hks, ITris, cmap = 'gray'):
    """
    Saves a mesh with vertex colors mapping to HKS values (visualizes one time scale).
    
    Args:
        filename (str): output path (.obj or .off).
        VPos (np.ndarray): [N, 3] vertices.
        hks (np.ndarray): [N] HKS values to map to color.
        ITris (np.ndarray): Connectivity.
    ...
    if filename.endswith('.obj'):
        save_obj(filename, VPos, C, ITris)
    else:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(input_path) and os.path.exists(os.path.join("input", args.input)):
        input_path = os.path.join("input", args.input)
        
    if input_path.endswith('.obj'):
        (VPos, VColors, ITris) = load_obj(input_path)
    else:
        (VPos, VColors, ITris) = load_off(input_path)
        
    neigvecs = min(VPos.shape[0], args.neigvecs)
    hks = get_hks(VPos, ITris, neigvecs, np.array([args.t]))
    
    # Output handling
    output_path = args.output
    if not os.path.dirname(output_path): # If just filename
        os.makedirs("output", exist_ok=True)
        if not output_path.startswith("output/"):
            output_path = os.path.join("output", output_path)
    
    saveHKSColors(output_path, VPos, hks[:, 0], ITris)
